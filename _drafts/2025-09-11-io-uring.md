...
date:   2025-09-11 20:00:00 +0800
categories: iouring
excerpt: ""
---

Inspired by <https://notes.eatonphil.com/2023-10-19-write-file-to-disk-with-io_uring.html>, I decided to dig deeper into `io_uring` as it's a potentially game changing Linux API for IO use cases
...
It didn't seem like there was much differences in writes. At this point, there's a possibility that my tests/the library used may not be written optimally. So I decided to try out `fio`.

`fio` is written by the same person who wrote `io_uring`. This will definitely the best representation.
...
Reads, on the other hand shows a completely different story. We can see that:
...
From this test, it seems like a naive usage of io_uring will help bring huge benefits to read workloads but not much for writes.

## Why are io_uring writes not as fast

> I don't have a good answer. These are guesses at best.

I tried to `strace` the write tests. Sadly, using strace slows down the sync test significantly. This is most likely due to the strace overhead caused by the no. of syscalls. See the [strace(s)](#straces) below for the traces

- sync has 500k syscalls, io_uring_basic has 380k, io_uring_batch only has 110k.

### My guess

Writes are super fast due to writes not hitting the disk as much (e.g. RAM cache). Because of this, the benefit from io_uring is not as apparent. Comparing this against reads where we really make the disk work hard, as seen by the lower IOPS. In such cases, the async nature of io_uring provided much obvious benefits (i.e. we don't need to busy-wait while data is fetched from disk)

## What's next

I will experiment integrating io_uring via the Go libraries to an open source database (with a focus on the read path) to see the benefits
